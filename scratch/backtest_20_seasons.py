...
def main():
    ...
    for target_season in target_seasons:
        ...
        for _, match in season_df.iterrows():
            # Check Chaos Blacklist
            mac_t = (match['home_tier'], match['away_tier'])
            mic_t = (match['home_micro'], match['away_micro'])
            teams = (match['home'], match['away'])
            
            if mac_t in [("T3", "T2"), ("T1", "T3"), ("T2", "T1"), ("T1", "T2"), ("T4", "T1")] or \
               mic_t in [("E", "D"), ("B", "A"), ("C", "A")] or \
               teams in [("Everton", "Manchester Blue"), ("Fulham", "West Ham"), ("Wolverhampton", "Tottenham")]:
                skipped_chaos += 1
    ...
    print(f"\n--- GRAND TOTAL (20 SEASONS w/ CHAOS FILTER) ---")
