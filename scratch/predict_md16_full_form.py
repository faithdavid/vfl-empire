...
# The first season's matches are the first 240 items in the array (30 matchdays * 8 matches)
...
for m in matches:
    if m.get('match_day') > 15: continue
    h = m['home_team']
    a = m['away_team']
...
print("### 🏆 FULL AUGMENTED LEAGUE TABLE (AFTER MD 15)")
print("| Rank | Team | Pts | W | D | L | GF | GA | GD | Full 15-Match Form String |")
print("| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |")
for rank, (team, stats) in enumerate(sorted_teams, 1):
    gd = stats['gf'] - stats['ga']
    full_form = get_full_form(team, 15)
    print(f"| **{rank}** | **{team}** | **{stats['pts']}** | {stats['w']} | {stats['d']} | {stats['l']} | {stats['gf']} | {stats['ga']} | {gd:+d} | `{full_form}` |")

print("\n### ⚽ MATCHDAY 16 FIXTURES")
md16 = [m for m in matches if m.get('match_day') == 16]
for i, fix in enumerate(md16, 1):
    h = fix.get('home_team')
    a = fix.get('away_team')
    print(f"{i}. {h} vs {a}")
