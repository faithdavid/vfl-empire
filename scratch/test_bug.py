...
group = df[(df['home'] == 'West Ham') & (df['away'] == 'London Guns') & (df['home_micro'] == 'E') & (df['away_micro'] == 'B')]
...
group['w_total'] = group['total'].astype(float) * pd.Series(weights).values
w_avg_goals = group['w_total'].sum() / sum_weight

print("w_avg_goals:", w_avg_goals)
