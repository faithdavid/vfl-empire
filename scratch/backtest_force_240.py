...
def main():
    ...
    for _, match in season_df.iterrows():
        ...
        for act_key, rate_key in markets:
            ...
            # Use Confluence score (average) or fallback to macro/micro if one is missing
            score = (mac_val + mic_val) / 2 if (mac_val > 0 and mic_val > 0) else max(mac_val, mic_val)
            
            # Heavy penalty for 1X2 and U15/O35 to force it to pick safer markets unless highly confident
    ...
    print(f"\n--- FORCED 240 PREDICTIONS (Season {target_season}) ---")
